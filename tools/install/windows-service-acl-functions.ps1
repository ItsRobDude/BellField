# BellField Windows service ACL function library.
#
# This file is dot-sourced by install-windows-services.ps1 and by the
# windows-service-acl.ps1 CLI wrapper. It must NEVER declare a top-level
# param() block: dot-sourcing a script that has one rebinds the caller's
# variables of the same names (empty strings for unbound [string] params),
# which is exactly the Gate Day rerun-24 failure where install-windows-services
# lost $ReleaseRoot after dot-sourcing the ACL helper. Standalone invocation
# with arguments belongs in windows-service-acl.ps1.
