...
export class CustomerDeliveryRepository {
  ...
  async createOutboundMessage(input: CreateOutboundMessageInput): Promise<OutboundMessageRecord> {
    await this.databaseService.query(
      `
    ...
    const message = await this.getOutboundMessageById(input.id);
    if (!message) {
  ...
  async hasRecentEstimateEmail(input: {
    estimateId: string;
    recipientEmail: string;
    since: string;
  }): Promise<boolean> {
    const result = await this.databaseService.query<{ id: string }>(
      `
        ...
        limit 1
      `,
      [input.estimateId, input.recipientEmail, input.since]
    );
