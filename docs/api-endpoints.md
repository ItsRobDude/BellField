...
| `POST` | `/operations/purchase-orders/:id/receive`        | office  | `purchasing:edit`   | Receive an ordered PO in full → inventory movements (parts) + equipment assets; equipment received to a job also posts a `receiveToJob` cost movement. Body `{ note?, lines?, confirmMissingSerial? }` (optional per-line actual qty/cost; confirm blank equipment serials). |
