...
| `GET`  | `/operations/inventory/field/truck-stock`              | field   | `register:create`   | The calling technician's pickable truck stock: active parts with positive on-hand on a truck location assigned to them (item, qty, weighted-average cost, location), for the field part-add picker. Scoped to the caller's own truck(s).                                                                                                                           |
