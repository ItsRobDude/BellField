...
| `GET`  | `/operations/estimates/:estimateId/send-preview`       | office  | `estimates:send`    | Resolve the send-panel preview for an approved estimate: fixed BellField From address, reply-to, rendered subject/body, and safe delivery readiness. Does not expose provider keys, provider names, or provider diagnostics.                                           |
...
| `POST` | `/operations/estimates/:estimateId/send`               | office  | `estimates:send`    | Send an approved, non-superseded estimate PDF by BellField-operated email from `estimates@bellfield.app`, writing a document snapshot, outbound message record, and timeline entry. Public response fields are sanitized.                                              |
