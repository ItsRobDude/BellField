...
| `GET`  | `/operations/jobs/:jobId/costing`                | office  | `jobCosting:view`   | Job cost read model: the always-current `live` rollup (inventory receiveToJob/issueToJob + labor + expense) plus the `finalized` snapshot frozen at completion (`isFinalized` flags whether a current snapshot exists).                                                                                                                                            |
