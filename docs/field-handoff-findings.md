...
- **Expired field sessions preserve unsynced work** — ordinary session expiry returns the technician to sign-in without wiping the local pending queue or staged media. After the same technician signs back in, their queued work remains available to sync.
- **Employee-scoped local field state** — pending operations, staged media, and disposable assigned-work/truck-stock/sync caches are scoped to the employee login that created them. A different technician login on the same tablet does not inherit, replace, drain, or replay another employee's queue.
...
| Photo/video/file queueing                | Present. Expo image/video capture or pick with app-owned local staging, a 50 MB client guard, SHA-256 metadata, upload-intent replay, raw blob finalization, and staged-file cleanup after sync. Real-device media smoke passed 2026-06-05. |
...
| Session expiry behavior                  | Present. Expired field sessions return to sign-in without wiping local unsynced work; same-employee re-login can continue syncing, while different-employee login cannot see or drain another employee's local queue.                       |
...
- Ordinary session expiry is not a destructive wipe path; revoked/inactive access remains destructive after the server confirms access loss.
- Pending operations and staged media stay scoped to the employee who created them.
