...
| D7  | Relay codebase location and hosting          | `apps/relay` in this monorepo, sharing `packages/contracts`; deployed separately, never in the release artifact. Hosting (decided 2026-06-11): pilot on the owner's home Unraid server as Docker containers behind a Cloudflare Tunnel (outbound-only, no inbound ports, home IP hidden); relay containers are single-purpose, image-pinned, with nightly off-box `pg_dump` and external uptime monitoring. A dedicated host or VPS is required before Phase 6 (acceptance/payment links) ships, because relay downtime becomes homeowner-visible there. | decided |
