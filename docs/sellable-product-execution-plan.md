...
| D7  | Relay codebase location and hosting          | `apps/relay` in this monorepo, sharing `packages/contracts`; deployed separately, never in the release artifact. Hosting (decided 2026-06-11, host updated in practice): deployed on the dedicated Triton laptop (Ubuntu) as Docker containers behind a Cloudflare Tunnel (outbound-only, no inbound ports, home IP hidden); relay containers are single-purpose, image-pinned, with nightly `pg_dump` (off-box target pending) and external uptime monitoring pending. Hosting revisit (decided 2026-06-12): the dedicated laptop hosts until the first paying customer has acceptance links live; no VPS or new hardware pre-revenue. Free hardening (uptime monitor, power-loss reboot proof, ethernet) instead — see `acceptance-links-design.md` and `relay-deployment-2026-06-12.md`. | decided |
