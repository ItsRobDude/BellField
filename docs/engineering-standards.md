...
- office UI primitives live in `apps/office-web/src/components/`: `FormField` for any input, `SubmitButton` + `useAsyncAction` for any button that writes, `ConfirmAction` / `ConfirmPanel` for anything that moves money or destroys data, `StatusMessage` for the outcome of an action; do not hand-roll labels, busy flags, `window.confirm`, or message boxes
