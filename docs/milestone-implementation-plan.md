...
- payments (`20260601_010` plus later allocation/refund migrations): online-only v1, manually recorded against posted invoices as an append-only ledger that never mutates invoice totals. Invoice-scoped payments apply to their selected posted charge invoice first, then spill to other open posted charges; null-source deposits stay job-level (unallocated credit before posted charges exist, main-first allocation after posted charges exist, and posting-time allocation to the main invoice for existing pre-post deposit credit); refund reversal applies the payment's source invoice first when one exists, then falls back to the job-level/main-first order. Amount due is derived (net billed − non-void payments + refunds). Record/list/void/refund are gated on the `payments` area, lock the invoice or payment rows as appropriate, and re-check state in-transaction; the job timeline gains payment recorded/voided/refunded events. Office payments and manual refund actions live in the Invoice tab.
- bookkeeping worklists: a read-only cross-job review surface (office Bookkeeping nav, `invoices:view`) — drafts ready to post, jobs with an open balance, and recently posted invoices, each paged with its true total and a Load more control.

All Milestone 8 migrations (007–010) and the full posting/adjustment/payment/balance/bookkeeping runtime were verified locally against the default dev database. Later communication/payment-depth slices have since added invoice PDF/email delivery, amount-scoped online payment links that default to the full amount due, job-level deposit links, posting-time allocation of existing pre-post deposit credit to the main invoice, manual office refunds, the provider-confirmed online refund backend path, source-aware refund allocation, the office Refund-on-card action + pending/failed display, the live Stripe sandbox payment-link depth smoke, and the live Stripe sandbox refund smoke. Still later: stored cards and deeper processor-fee reconciliation. Customer card surcharge / processing-fee pass-through is intentionally not planned for v1 unless real customer demand justifies a dedicated legal and card-network review.
