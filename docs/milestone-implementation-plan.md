...
All Milestone 8 migrations (007–010) and the full posting/adjustment/payment/balance/bookkeeping runtime were verified locally against the default dev database. Later communication/payment-depth slices have since added invoice PDF/email delivery, amount-scoped online payment links that default to the full amount due, manual office refunds, the provider-confirmed online refund backend path, the office Refund-on-card action + pending/failed display, and the live Stripe sandbox refund smoke. Still later: deposits, stored cards, and deeper processor-fee reconciliation.
