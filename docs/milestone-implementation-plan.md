...
All Milestone 8 migrations (007–010) and the full posting/adjustment/payment/balance/bookkeeping runtime were verified locally against the default dev database. Later communication/payment-depth slices have since added invoice PDF/email delivery, full-balance online payment links, manual office refunds, the provider-confirmed online refund backend path, and the office Refund-on-card action + pending/failed display. Still later: the online refund live Stripe sandbox smoke, partial payments, deposits, stored cards, and deeper processor-fee reconciliation.
