...
- invoice posting/locking, adjustment/credit corrections, manually recorded payments, manual office refunds, invoice PDF/email delivery, amount-scoped online payment links that default to the full amount due, job-level deposit links, BellField-branded online payments setup from Settings, and the provider-confirmed online refund path (backend + office Refund-on-card action, pending/failed display, and live Stripe sandbox smokes for payment links/refunds) have shipped; the office surfaces a job balance and amount due plus a read-only cross-job bookkeeping worklist
- invoice-scoped payments apply to the selected posted charge invoice first, then spill to other open posted charges; null-source deposits stay job-level (unallocated credit before posted charges exist, main-first allocation after posted charges exist, and posting-time allocation to the main invoice for existing pre-post deposit credit); refund reversal applies the payment's source invoice first when one exists, then falls back to the job-level/main-first order for remaining reversible allocations
- still later: stored cards, SMS, customer portal behavior, and deeper processor-fee reconciliation. Customer card surcharge / processing-fee pass-through is intentionally not planned for v1 unless real customer demand justifies a dedicated legal and card-network review
