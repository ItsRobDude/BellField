...
| 9   | Estimates, pricing, register & invoice draft                         |   8 | O/C   | Estimates attach to jobs and stay visible from locations. Office/field register entries (labor, parts, service items, memberships, other) feed the invoice draft; office reviews/edits draft lines. Multiple estimates and zero-dollar invoices handled cleanly. **Pricebook / flat-rate task catalog**, good/better/best options, and recurring membership/service-agreement depth are major FSM differentiators; grade those as Track A parity unless BellField has explicitly brought them into current scope. |
...
- Estimate options (good/better/best), estimate PDF export, customer acceptance
...
- Customer portal
- **Pricebook / flat-rate task catalog** (BellField prices ad-hoc lines via a pricing engine; no task catalog)
- **Memberships / service agreements / recurring maintenance** (`membership` exists only as a register line _kind_, no lifecycle)
...
7. **Missing FSM features surfaced** — pricebook/flat-rate, memberships/agreements, on-my-way notifications — embedded in the relevant rows plus a dedicated _Out-of-scope_ callout.
