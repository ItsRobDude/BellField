...
| Runner "idle" with no output, no evidence anywhere, PIDs killed/lost                         | Bad START-HERE command (missing required args → invisible mandatory-parameter prompt in a hidden/elevated window). Validate the USB's START-HERE against the checklist commands before blaming product or operator                                                                                                  | PR #92 (usage fail-fast, validate-start-here)                |
| Operator improvised at a documented step (wrong credential, wrong flow)                      | START-HERE briefing content regressed on rebuild — the operator read only START-HERE, which lost institutional content (docs pointers, dummy credential, human-action steps). Check what docs the report says were read                                                                                             | PR #93 (auto first-owner, briefing content rules)            |
| Run PASSES but gates remain unattempted; report says "not in the active START-HERE sequence" | Automation frontier reached — the checklist documents gates the runner and START-HERE never carried, so the strict run ran out of paved road, not luck. Diff checklist gates against the runner's ValidateSet before celebrating; validate-start-here now fails the USB build when the sequence stops before Gate 4 | PR #94 (gate4-expired-refusal mode, full-sequence validator) |
